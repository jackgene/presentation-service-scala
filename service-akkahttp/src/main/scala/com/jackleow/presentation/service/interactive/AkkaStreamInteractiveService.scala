...
//
//      Source.
//        lazySingle { () => Option(lastEmitted.get) }.
//        collect {
...
class AkkaStreamInteractiveService(
  ...
  private def sendersByTokenCounter(
    ...
    Flow[ChatMessage | Reset.type].
      ...
      scan(
        (
          Map[String, FifoBoundedSet[String]]().withDefaultValue(FifoBoundedSet(maxTokensPerSender)),
          MultiSet[String]()
        )
      ) {
        case (
  ...
  override def receiveChatMessage(chatMessage: ChatMessage): Future[Unit] =
    Future.
      successful { inputQueue.offer(chatMessage) }.
      filter(_.isEnqueued).
      map(_ => ())

  override def reset(): Future[Unit] =
    Future.
      successful { inputQueue.offer(Reset) }.
      filter(_.isEnqueued).
      map(_ => ())
